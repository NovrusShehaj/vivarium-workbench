...
export default function DetailMenu(props: {
  ...
  const seg = (opts: { id: string; label: string }[], value: string, onPick: (id: string) => void) => (
    <div style={{ display: 'inline-flex', border: '1px solid var(--border-2, #d1d5db)', borderRadius: 5, overflow: 'hidden' }}>
      {opts.map((o, i) => (
        <button
          key={o.id}
          onClick={() => onPick(o.id)}
          style={{
            ...
            borderLeft: i === 0 ? 0 : '1px solid var(--border, #e5e7eb)',
            background: value === o.id ? '#0d6e6b' : 'var(--surface, #fff)',
            color: value === o.id ? '#fff' : 'var(--text, #374151)',
  ...
  const rowLabel: React.CSSProperties = { fontSize: 11.5, color: 'var(--text, #374151)', width: 58, flexShrink: 0 };
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}>
      <button
        ...
        style={{
          height: 28, padding: '0 10px', fontSize: 12, background: (open || anyForced) ? 'var(--accent-bg, #ecfdf9)' : 'var(--surface, #fff)',
          display: 'inline-flex', alignItems: 'center', gap: 5,
          border: '1px solid var(--border-2, #d1d5db)', borderRadius: 4, cursor: 'pointer',
          color: (open || anyForced) ? 'var(--accent-text, #0d6e6b)' : 'var(--text, #374151)', fontWeight: anyForced ? 600 : 400,
        }}
      >
        Detail ▾
      </button>
          ...
          <div style={row}>
            ...
            <div style={{ display: 'inline-flex', border: '1px solid var(--border-2, #d1d5db)', borderRadius: 5, overflow: 'hidden' }}>
              <button onClick={() => bumpFont(-0.1)} title="Smaller text"
                style={{ padding: '3px 9px', fontSize: 12, fontWeight: 700, border: 0, borderRight: '1px solid var(--border, #e5e7eb)', background: 'var(--surface, #fff)', color: 'var(--text, #374151)', cursor: 'pointer' }}>A−</button>
              <button onClick={() => setFontScale(1)} title="Reset text size"
                style={{ padding: '3px 9px', fontSize: 11, minWidth: 46, border: 0, background: 'var(--surface, #fff)', color: fontScale !== 1 ? 'var(--accent-text, #0d6e6b)' : 'var(--text-subtle, #6b7280)', fontWeight: fontScale !== 1 ? 700 : 400, cursor: 'pointer' }}>{fontScale.toFixed(2)}×</button>
              <button onClick={() => bumpFont(0.1)} title="Larger text"
                style={{ padding: '3px 9px', fontSize: 15, fontWeight: 700, border: 0, borderLeft: '1px solid var(--border, #e5e7eb)', background: 'var(--surface, #fff)', color: 'var(--text, #374151)', cursor: 'pointer' }}>A+</button>
            </div>
          </div>
          <div style={{ height: 1, background: 'var(--surface-3, #e5e7eb)', margin: '4px 0' }} />
          <button
            ...
            style={{
              ...
              padding: '7px 12px', fontSize: 12, border: 0, background: 'var(--surface, #fff)',
              cursor: anyForced ? 'pointer' : 'default', color: anyForced ? 'var(--text, #374151)' : '#b8bec9',
            }}
            onMouseEnter={(e) => { if (anyForced) e.currentTarget.style.background = 'var(--surface-3, #f3f4f6)'; }}
